    ...
    (function ($, window, document, undefined) {
        ...
        Plugin.prototype = {
            init: function () {
                ...
                if (this.options.initialDialCode && (this.telInput.val() === "" || this.telInput.val() == $(this.telInput).attr('placeholder'))) {
                ...
                if (this.options.autoHideDialCode) {
                    this.telInput.focusin(function () {
                        ...
                        if (value.length === 0 || value == $(that.telInput).attr('placeholder')) {
         ...
         ********************/
            ...
            _selectListItem: function (listItem) {
                ...
                // focus the input
                this.telInput.focus();
                // workaround fo IE focus
                var tmpStr = this.telInput.val();
                this.telInput.val('');
                this.telInput.val(tmpStr);
            },
            ...
            _updateNumber: function (dialCode) {
                var inputVal = (this.telInput.val() == $(this.telInput).attr('placeholder')) ? '' : this.telInput.val();
         ...
         ********************/
            ...
            setDefaultCountry: function (countryCode) {
                ...
                if (!this.selectedFlagInner.hasClass(countryCode)) {
                    this._selectFlag(countryCode);
                    var countryData = this._getCountryData(countryCode, false);
                    this.defaultCountry = countryData;
                }
            }
        };
       ...
       ********************/
        ...
        $.fn[pluginName].getDialCode = function (inputVal) {
            var firstPart = $.trim(inputVal).split(" ")[0];
            // only interested in international numbers (starting with a plus)
            if (firstPart.substring(0, 1) == "+") {
                // strip out non-numeric chars (e.g. pluses, spaces, brackets)
                ...
                for (var i = dialCode.length; i > 0; i--) {
                    dialCode = dialCode.substring(0, i);
                    // if we find a match (a valid dial code), then return the dial code
                    if (intlDataFull.countryCodes[dialCode]) {
                        return dialCode;
                    }
                }
            }
